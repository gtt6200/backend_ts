...
const correoSchema = z.object({
    ...
    emailUser: z.string().email({ message: 'Email must be a type email.' }),
    mensaje: z.string({ invalid_type_error: 'Message is required.', required_error: 'Message is required.' }),
    emailService: z.string({ invalid_type_error: 'Email must be a type email.', required_error: 'Email is required.' }).email({ message: 'Email must be a type email.' })
});
